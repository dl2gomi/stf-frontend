  ...
  return (
    <>
      <nav id="mobile-main-nav" className="mobile-main-nav">
        <div className="flex mt-5 mb-4 w-100" onClick={() => open()}>
          <div id="wallet-header" className="d-flex w-100">
            <div id="connectbtn" className="tf-button style-1 w-100">
              <span>{isConnected ? `${address.slice(0, 4)}...${address.slice(-4)}` : 'Wallet Connect'}</span>
              <i className="icon-wa" />
            </div>
          </div>
        </div>
        <ul id="menu-mobile-menu" className="menu mt-0 pt-0">
          ...
          {isConnected && (
            <li className="menu-item">
              <Link className="item-menu-mobile" href="/dashboard">
                Dashboard
              </Link>
            </li>
          )}
